...
public:
  ...
  byte toon_index;
  byte edge_flag;
  uint32_t face_vert_count; // polygon No.0 ~ polygon No.(pmdMaterial[0].face_vert_count/3 - 1) has material No.0. And so on.
  byte texture_file_name[20];
  PmdMaterial() {
    for (int i = 0; i < 20; ++i) texture_file_name[i] = 0;
  }
};
